...
class MovieWatchingModel extends BaseModel{
    async getListWatching(id){
        let sql = `SELECT m.id AS movie_id, e.id AS episode_id, e.name AS episodeName, e.moviesUrl
                    FROM Movies m
                    JOIN Episodes e ON m.id = e.moviesId
                    WHERE m.id = ${id};`
        return await this.querySql(sql)
    }
    async getEpisode(epsId,movieId){
        ...
        WHERE Movies.id=${movieId} AND Episodes.id=${epsId}
      `
        return await  this.querySql(sql);
    }
    async getComment(id){
